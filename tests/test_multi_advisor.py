...
def _make_advisor(claude_response: str, chatgpt_response: str) -> MultiLLMAdvisor:
...
def test_primary_falls_back_to_claude_when_no_majority():
    ...
    advisor = _make_advisor(_BUY_JSON, _HOLD_JSON)
...
def test_primary_uses_majority_vote_picks_highest_confidence():
    # Both say buy AAPL; chatgpt has higher confidence → chatgpt is primary
    advisor = _make_advisor(_BUY_LOW_JSON, _BUY_HIGH_JSON)
    result = asyncio.run(advisor.analyze("headline", "summary", ["AAPL"], set(), set(), 0.0))
    assert result.primary.action == "buy"
    assert result.primary.ticker == "AAPL"
    assert result.primary.confidence == 0.95
    assert result.primary_provider == "chatgpt"


def test_primary_uses_majority_vote_both_agree():
    # Both claude and chatgpt say buy — majority of 2
    _BUY_CLAUDE = '{"action":"buy","ticker":"TSLA","reasoning":"catalyst","confidence":0.85,"hold_hours":4}'
    _BUY_CHATGPT = '{"action":"buy","ticker":"TSLA","reasoning":"catalyst","confidence":0.80,"hold_hours":4}'
    advisor = _make_advisor(_BUY_CLAUDE, _BUY_CHATGPT)
...
def test_all_results_ordered_claude_chatgpt():
    advisor = _make_advisor(_BUY_JSON, _HOLD_JSON)
    result = asyncio.run(advisor.analyze("headline", "summary", ["AAPL"], set(), set(), 0.0))
...
def test_partial_provider_error_still_returns_two_results():
    advisor = object.__new__(MultiLLMAdvisor)
    claude_mock = AsyncMock()
    claude_mock.complete.return_value = _make_completion(_BUY_JSON)
    chatgpt_mock = AsyncMock()
    chatgpt_mock.complete.side_effect = RuntimeError("timeout")
    advisor._claude = claude_mock
    advisor._chatgpt = chatgpt_mock
    advisor._claude_model = "claude-haiku-4-5"
    advisor._chatgpt_model = "gpt-5.4-mini"
    ...
    assert result.all_results[1].provider == "chatgpt"
