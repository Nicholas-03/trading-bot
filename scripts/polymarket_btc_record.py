...
with the local receive time t (ms) on every row, to data/btc_live/<start>.jsonl:
  ...
  {"k": "bk", "t", "ts", "a", "b", "bs", "x", "xs"}  best bid, its size, best ask, its size of an outcome token when they
                                      change (ts = Polymarket's server time of the change)
"""
import argparse
import asyncio
import json
...
class Recorder:
    def __init__(self, path: Path):
        ...
        self.last_bn = 0
        self.version, self._tok_ver, self._tok_key, self._toks = 0, -1, -1, []
    ...
    def tokens(self) -> list[str]:
        if self._tok_key != len(self.markets) or self._tok_ver != self.version:
            self._toks = sorted(x for m in self.markets.values() for x in (m["up"], m["down"]))
            self._tok_key, self._tok_ver = len(self.markets), self.version
        return self._toks

    def book_event(self, tok: str, ts: str) -> None:
        ...
        if state != self.best.get(tok):
            self.best[tok] = state
            self.write({"k": "bk", "t": now_ms(), "ts": int(ts), "a": tok[-8:], "b": b, "bs": state[1], "x": x, "xs": state[3]})
        ...
        while True:
            ...
            try:
                async with websockets.connect("wss://ws-subscriptions-clob.polymarket.com/ws/market", ping_interval=10,
                    ...
                    async for raw in ws:  # the feed never goes quiet, so the market check below runs often enough
                        if self.tokens() != toks:
                            break
                        msg = json.loads(raw)
                        for e in msg if isinstance(msg, list) else [msg]:
                            if "bids" in e and "asks" in e:
                                ...
                                self.book_event(e["asset_id"], e["timestamp"])
                            for c in e.get("price_changes", []):
                                ...
                                if s:
                                    ...
                                    book.pop(p, None)
                                self.book_event(c["asset_id"], e["timestamp"])
